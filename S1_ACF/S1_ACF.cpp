// S1_ACF.cpp : DLL 응용 프로그램을 위해 내보낸 함수를 정의합니다.
//
...
localMxArray2CvMat(mxArray* I, T type)
{
	...
	{            		
		...
		{ 
			for (int i = 0; i < h; i++, pixelOffset++)
			{
				matI.row(i).col(j).data[nChannel - 1 - ch] = ptr[pixelOffset];	// 이게 지금 float인데 UINT8로 선언되어있음.....
			}
		}
	}
	return matI;
